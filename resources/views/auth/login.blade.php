        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="flex items-center text-center justify-end mt-4">
                @if (Route::has('password.request'))
                    <a class="flex-1 text-left underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('password.request') }}">
                        {{ __('Forgot your password?') }}
                    </a>
                @endif

                <a class="flex-1 text-right underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('register') }}">
